...
export function enregistrerHandlersClients(): void {
  ipcMain.handle('clients:lister', () => {
    return getDb().prepare('SELECT * FROM clients ORDER BY nom').all() as unknown as Client[]
  })

  ipcMain.handle('clients:ajouter', (_e, client: Omit<Client, 'id'>) => {
  ...
  ipcMain.handle('clients:modifier', (_e, client: Client) => {
    const erreur = validerClient(client)
    if (erreur) throw new Error(erreur)
  ...
  ipcMain.handle('clients:supprimer', (_e, id: number) => {
  ...
  ipcMain.handle('clients:obtenirDetail', (_e, id: number) => {
    ...
    const lignesDevis = getDb()
      .prepare('SELECT id, numero, date, statut, remise_pct, tva_pct FROM devis WHERE client_id = ? ORDER BY date DESC, id DESC')
      .all(id) as unknown as {
      id: number
      numero: string
      date: string
