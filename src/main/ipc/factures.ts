...
function validerFacture(facture: Omit<Facture, 'id' | 'stockDeduit'>): string | null {
  ...
  if (facture.remisePct < 0 || facture.remisePct > 100) return 'La remise doit être comprise entre 0 et 100%.'
  if (facture.tvaPct < 0 || facture.tvaPct > 100) return 'La TVA doit être comprise entre 0 et 100%.'
  return null
}

function chargerDetail(id: number): FactureDetail {
...
export function enregistrerHandlersFactures(): void {
  ipcMain.handle('factures:prochainNumero', () => {
  ...
  ipcMain.handle('factures:creerBrouillon', (_e, clientId: number) => {
  ...
  ipcMain.handle('factures:obtenirDetail', (_e, id: number) => chargerDetail(id))

  ipcMain.handle('factures:dupliquer', (_e, id: number) => {
    ...
    return dansUneTransaction(() => {
      ...
      for (const ligne of source.lignes) {
        insererLigne.run(nouvelId, ligne.designation, ligne.referenceInventaire, ligne.quantite, ligne.prixUnitaire)
      }
  ...
  ipcMain.handle('factures:creerDepuisDevis', (_e, devisId: number) => {
    ...
    const lignesDevis = db
      .prepare('SELECT designation, quantite, prix_unitaire FROM devis_lignes WHERE devis_id = ? ORDER BY id')
  ...
  ipcMain.handle('factures:enregistrer', (_e, detail: FactureDetail) => {
  ...
  ipcMain.handle('factures:supprimer', (_e, id: number) => {
  ...
  ipcMain.handle('factures:historique', () => {
    const lignes = getDb()
      .prepare(
      ...
      .all() as unknown as (LigneFactureDb & { client_nom: string | null; montant: number | null })[]
    ...
    return lignes.map((ligne): HistoriqueFacture => {
      ...
      if (facture.statut === 'En attente') {
        joursEnAttente = Math.floor((maintenant - new Date(facture.date).getTime()) / (1000 * 60 * 60 * 24))
      }
      return { ...facture, clientNom: ligne.client_nom ?? 'Client supprimé', montant: ligne.montant, joursEnAttente }
    })
  })
  ...
  ipcMain.handle('factures:confirmerEnregistrementHistorique', (_e, id: number) => {
    ...
    const fraisImpression = detail.impressionIncluse
      ...
      : 0
    const { total } = calculerTotalDocument(detail.lignes, detail.remisePct, detail.tvaPct, fraisImpression)
