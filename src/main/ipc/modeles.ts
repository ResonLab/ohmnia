...
export function enregistrerHandlersModeles(): void {
  ...
  ipcMain.handle('modeles:creer', (_e, nom: string) => {
    ...
    const resultat = getDb()
      .prepare('INSERT INTO modeles_prestations (nom) VALUES (?)')
      .run(nom.trim())
    tracerAudit('creation', 'modele', nom.trim())
    return chargerModele(Number(resultat.lastInsertRowid))
  })

  ipcMain.handle('modeles:enregistrer', (_e, modele: ModelePrestation) => {
  ...
  ipcMain.handle('modeles:supprimer', (_e, id: number) => {
