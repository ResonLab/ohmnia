...
/** Journal d'audit et clôtures d'exercice. Aucune dépendance à Electron. */
...
export function listerAudit(limite = 300): EntreeAudit[] {
  const lignes = getDb()
...
export function viderAudit(): void {
...
export function listerExercices(): ExerciceCloture[] {
  const lignes = getDb()
...
export function cloturerExercice(annee: number): number {
...
export function reouvrirExercice(annee: number): void {
