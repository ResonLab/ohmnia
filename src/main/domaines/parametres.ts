...
/**
 * Paramètres de calcul (marge, déplacement, impression) et charges fixes,
 * sans Electron. Voir `./clients.ts` pour la règle : rien de la fenêtre ici.
 *
 * À ne pas confondre avec `./parametresApp.ts`, qui porte les réglages de
 * l'application elle-même (thème, langue, sauvegardes).
 */
...
export function lireParametresMarge(): ParametresMarge {
  const ligne = getDb()
...
export function enregistrerParametresMarge(valeurs: ParametresMarge): ParametresMarge {
  const erreur =
...
export function lireParametresDeplacement(): ParametresDeplacement {
  const ligne = getDb()
...
export function enregistrerParametresDeplacement(
  valeurs: ParametresDeplacement
): ParametresDeplacement {
  const erreur =
...
export function lireParametresImpression(): ParametresImpressionDb {
  const ligne = getDb()
...
export function enregistrerParametresImpression(
  valeurs: ParametresImpressionDb
): ParametresImpressionDb {
  getDb()
    .prepare(
...
export function listerChargesFixes(): ChargeFixe[] {
  const lignes = getDb()
...
export function ajouterChargeFixe(charge: Omit<ChargeFixe, 'id'>): ChargeFixe {
...
export function modifierChargeFixe(charge: ChargeFixe): ChargeFixe {
  if (!charge.libelle.trim()) throw new Error('Le libellé de la charge est obligatoire.')
  const erreur = validerMontantPositif(charge.montantMensuel, 'Le montant mensuel')
  if (erreur) throw new Error(erreur)
...
export function supprimerChargeFixe(id: number): void {
