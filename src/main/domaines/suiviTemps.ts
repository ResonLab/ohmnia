...
export function listerInterventions(): Intervention[] {
  const lignes = getDb()
...
export function interventionEnCours(): Intervention | null {
  const ligne = getDb()
...
export function demarrerIntervention(description: string, clientId: number | null): Intervention {
...
export function arreterIntervention(id: number): Intervention {
...
export function modifierIntervention(intervention: Intervention): Intervention {
...
export function supprimerIntervention(id: number): void {
 ...
 */
export function facturerInterventions(
  ids: number[],
  factureId: number,
  tauxHoraireParDefaut: number
): number {
  const db = getDb()
  ...
  return dansUneTransaction(() => {
    ...
    for (const id of ids) {
      const ligne = db
        .prepare(
          'SELECT description, secondes_ecoulees, fin, facture_id, taux_horaire FROM suivi_temps WHERE id = ?'
        )
        .get(id) as
        | {
            description: string
      ...
      insererLigne.run(factureId, ligne.description || "Main d'œuvre", Number(heures.toFixed(2)), taux)
