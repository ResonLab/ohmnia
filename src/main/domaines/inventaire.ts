...
export function listerInventaire(): ArticleInventaire[] {
  const lignes = getDb()
...
export function referenceSuggeree(): string {
  const lignes = getDb()
...
export function ajouterArticle(article: ArticleInventaire): ArticleInventaire {
  const erreur = validerArticle(article)
  if (erreur) throw new Error(erreur)

  const existe = getDb().prepare('SELECT 1 FROM inventaire WHERE reference = ?').get(article.reference)
...
export function modifierArticle(
...
export function supprimerArticle(reference: string): void {
  getDb().prepare('DELETE FROM inventaire WHERE reference = ?').run(reference)
}

export function resumeInventaire(): ResumeInventaire {
