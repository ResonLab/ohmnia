...
export function chargerDetailDevis(id: number): DevisDetail {
  const ligne = getDb().prepare('SELECT * FROM devis WHERE id = ?').get(id) as unknown as
    | LigneDevisDb
    | undefined
...
export function prochainNumeroDevis(): string {
...
export function creerBrouillonDevis(clientId: number): DevisDetail {
  ...
  return chargerDetailDevis(Number(resultat.lastInsertRowid))
}

export function dupliquerDevis(id: number): DevisDetail {
  const source = chargerDetailDevis(id)
  const db = getDb()
  ...
  return dansUneTransaction(() => {
    ...
    return chargerDetailDevis(nouvelId)
  })
}

export function enregistrerDevis(detail: DevisDetail): DevisDetail {
  ...
  return chargerDetailDevis(detail.id)
}

export function supprimerDevis(id: number): void {
  getDb().prepare('DELETE FROM devis WHERE id = ?').run(id)
}

export function changerStatutDevis(id: number, statut: Devis['statut']): void {
  getDb().prepare('UPDATE devis SET statut = ? WHERE id = ?').run(statut, id)
}

export function historiqueDevis(): HistoriqueDevis[] {
  const lignes = getDb()
    .prepare(
    ...
    .all() as unknown as (LigneDevisDb & {
    client_nom: string | null
    facture_liee: string | null
  })[]
