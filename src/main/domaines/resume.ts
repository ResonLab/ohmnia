...
/** Résumé annuel et objectif de chiffre d'affaires. Aucune dépendance à Electron. */
...
export function resumeParAnnee(): ResumeAnnuel[] {
  const lignes = getDb()
    .prepare(
...
export function lireObjectifAnnuel(annee: number): number {
  const ligne = getDb()
...
export function enregistrerObjectifAnnuel(annee: number, objectifCa: number): number {
