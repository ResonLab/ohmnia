...
import type { MouvementBancaire } from '../../shared/types'

/**
 * Comptabilité — la partie qui ne dépend pas d'Electron : construction du CSV,
 * analyse des relevés bancaires, rapprochement et import.
 *
 * Ce qui reste dans `../ipc/comptabilite.ts` : choisir le fichier à lire et
 * l'endroit où écrire l'export. Le reste, y compris les analyseurs, vit ici.
 *
 * **Le bloc entre « Découpe une ligne CSV » et « Marque les mouvements » est
 * lu tel quel par `tests/parseurs-bancaires.mjs`**, qui en retire les types
 * pour l'exécuter. Il ne doit donc rien contenir qui touche la base ou
 * Electron, et ses fonctions restent internes au module : le test ajoute ses
 * propres `export`.
 */
...
/**
 * Analyse le contenu d'un relevé, quel que soit son format, et le rapproche
 * du Journal. Lever l'erreur ici plutôt que dans la couche fenêtre garantit
 * le même message que le relevé arrive par un sélecteur de fichiers ou,
 * plus tard, par le réseau.
 */
export function analyserReleve(contenu: string, estXml: boolean): MouvementBancaire[] {
